...
const transform: AxiosTransform = {
    ...
    transformRequestHook: (res, options) => {
        const {isTransformResponse, isReturnNativeResponse, needToken} = options;
    ...
    beforeRequestHook: (config, options) => {
        ...
        const {apiUrl, isJoinPrefix, urlPrefix, joinParamsToUrl, formatDate, joinTime = true, needToken} = options;
        // 添加接口前缀
    ...
    requestInterceptors: (config, options) => {
        // 请求之前处理config
        // const {needToken} = config.requestOptions;
        // if (needToken) {
